...
ForEach ($Session in $Sessions){
    ...
    If ($QueryCheck -match "$DataCollectorSet") {
        ...
        If ($Status -match 'Stopped') {
            ...
            Invoke-Command -Session $Session -ScriptBlock {Param($DataCollectorSet, $FilePath, $SessionName);logman create counter $DataCollectorSet -o "$FilePath\$SessionName.blg" -c "\Processor(_Total)\% Processor time","\Memory\Available MBytes","\PhysicalDisk(0 C:)\Disk Read Bytes/sec","\PhysicalDisk(0 C:)\Disk Write Bytes/sec","\Network Adapter(Hyper-V Virtual Ethernet Adapter)\Bytes Sent/sec","\Network Adapter(Hyper-V Virtual Ethernet Adapter)\Bytes Received/sec" -rf 00:05:00 -si 00:00:01 -max 1024 -f bincirc --v -ow} -ArgumentList $DataCollectorSet, $FilePath, $SessionName
